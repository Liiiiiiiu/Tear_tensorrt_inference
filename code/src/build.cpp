...
std::shared_ptr<Model> build_model(char **argv) {
    ...
    // else if (model_arch == "yolov8Pose")
    //     model = std::make_shared<YOLOv8Pose>(root[model_arch]);
    else if (model_arch == "resnet")
        model = std::make_shared<Resnet>(root[model_arch]);
    // else if (model_arch == "ResnetAgender")
    //     model = std::make_shared<ResnetAgender>(root[model_arch]);
    // else if (model_arch == "PlateUS")
    //     model = std::make_shared<PlateUS>(root[model_arch]);
    else if (model_arch == "yolov5_cu")
        model = std::make_shared<yolov5_cu>(root[model_arch]);
    else
